...
export default function Navbar() {
  ...
  return (
    <nav className="fixed left-0 right-0 min-h-12 p-4 shadow-sm">
      ...
      <div className="lg:hidden flex items-center shadow-sm p-4">
        {(isOpen && (
          <X size={32} className="z-20" onClick={() => setIsOpen(false)} />
        )) || (
          <Menu size={32} className="z-20" onClick={() => setIsOpen(true)} />
        )}
        <h1 className="text-xl font-bold w-full text-center">SIKAT</h1>
        {isOpen && (
          <ul className="absolute px-4 py-24 flex flex-col justify-around min-h-screen left-0 right-0 top-0 bottom-0">
